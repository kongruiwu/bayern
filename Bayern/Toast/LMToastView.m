...
//for 自动化测试
@property (nonatomic, strong) id<LMToastDelegate>toastDelegate;
...
+ (LMToastView *)presentToastWithin:(UIView *)superview withIcon:(APToastIcon)icon text:(NSString *)text duration:(NSTimeInterval)duration
{
...
+ (LMToastView *)presentToastWithin:(UIView *)superview withIcon:(APToastIcon)icon text:(NSString *)text duration:(NSTimeInterval)duration completion:(void (^)())completion
{
...
{
	LMToastView *toast = [[LMToastView alloc] initWithText:text toastType:icon];
...
{
    LMToastView *toast = [[LMToastView alloc] initWithText:text toastType:APToastIconLoading];
    [superview addSubview:toast];
...
{
    LMToastView *toast = [[LMToastView alloc] initWithText:text toastType:APToastIconNone];
    UIView *backgroundView = [[UIView alloc] initWithFrame:superview.bounds];
    [superview addSubview:backgroundView];
    [backgroundView addSubview:toast];
    toast.backgoundView = backgroundView;
...
- (BOOL)relatedToToast:(LMToastView *)toast
{
...
{
	for (LMToastView *toast in self.window.toastStack) {
		if (toast == self) {
			break;
		}
...
- (void)setToastViewPosition:(LMToastView *)toast
{
